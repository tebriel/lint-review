...
class TestReview(TestCase):
    ...
    def test_load_comments__none_active(self):
        ...
        self.pr.review_comments.return_value = [
            GhIssueComment.from_dict(f, self.session) for f in json.loads(fixture_data)
        ]

        review = Review(self.repo, self.pr, self.config)
        review.load_comments()

        self.assertEqual(0, len(review.comments("View/Helper/AssetCompressHelper.php")))
    ...
    def test_publish_status__has_errors__success_status(self):
        ...
        tst_config = build_review_config(fixer_ini, app_config)
        self.assertEqual('success', tst_config.failed_review_status(), 'config object changed')
    ...
    def test_publish_review_empty_comment_with_comment_status(self):
        tst_config = build_review_config(fixer_ini, {'PULLREQUEST_STATUS': True})
    ...
    def test_publish_review_no_count_change(self):
        ...
        tst_config = build_review_config(fixer_ini, {'SUMMARY_THRESHOLD': 1})
        with patch('lintreview.review.Review.publish_summary'):
            with patch('lintreview.review.Review.publish_status') as pub_status_mock:
                review = Review(self.repo, self.pr, tst_config)
...
"""
    ...
    def test_publish_checkrun__has_errors_force_success_status(self):
        self.repo.create_checkrun = Mock()
        tst_config = build_review_config(fixer_ini, {'PULLREQUEST_STATUS': False})
        self.assertEqual('success', tst_config.failed_review_status(), 'config object changed')
...
class TestProblems(TestCase):
    ...
    def test_add__with_diff_containing_block_offset(self):
        ...
        first_result: Comment = result[0]
        self.assertIsInstance(first_result, Comment)
        self.assertEqual(changes.line_position('somefile.py', line_num), first_result.position,
